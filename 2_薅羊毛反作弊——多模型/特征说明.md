...
系统从原始17个字段中提取**53维特征向量**，用于模型训练和预测。特征提取器通过规则运算、统计计算、哈希转换等方式将原始数据转换为数值特征。
...
数值特征向量（53维）
    ↓
机器学习模型（XGBoost + RandomForest + GBDT + LightGBM + IsolationForest）
```
...
| 13 | `mouse_trajectory_entropy` | 直接使用原始值 | 鼠标轨迹熵值（0-1） |
| 14 | `request_frequency` | 直接使用原始值 | 请求频率（次/秒） |
